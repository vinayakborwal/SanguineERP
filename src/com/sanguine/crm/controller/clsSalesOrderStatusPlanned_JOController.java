...
@Controller
public class clsSalesOrderStatusPlanned_JOController {
	...
	// Open Intent Status
	@RequestMapping(value = "/frmPlanned_JO", method = RequestMethod.GET)
	public ModelAndView funOpenSOIntentStausModal(@RequestParam("soCode") String soCode, HttpServletRequest req, HashMap<String, Object> model) {
		...
		model.put("tableRows", funSetDataSOStatusPlanned_JOModal(soCode, req));
		return new ModelAndView("frmPlanned_JO");
	}

	@SuppressWarnings("rawtypes")
	public ArrayList<Object> funSetDataSOStatusPlanned_JOModal(@RequestParam("soCode") String soCode, HttpServletRequest req) {
		...
		// String
		// sqlData="SELECT a.strJOCode, DATE_FORMAT(a.dteJODate,'%d-%m-%Y'),b.strProdCode,f.strProdName, ROUND(a.dblQty,2) ,b.strJACode, DATE_FORMAT(c.dteJADate,'%d-%m-%Y'), ROUND(b.dblQty,2),c.strSCCode,g.strPName, d.strDNCode, "
		// +" DATE_FORMAT(d.dteDNDate,'%d-%m-%Y'), DATE_FORMAT(d.dteExpDate,'%d-%m-%Y'), ROUND(d.dblTotal,2),e.strSRCode, b.strNatureOfProcessing, DATE_FORMAT(e.dteSRDate,'%d-%m-%Y'), ROUND(h.dblAcceptQty,2) "
		// +" FROM tbljoborderhd a left join tbljoborderallocationdtl b on a.strJOCode=b.strJOCode "
		// +" left join  tbljoborderallocationhd c on  b.strJACode=c.strJACode "
		// +" left join tbldeliverynotehd d on b.strJACode=d.strJACode "
		// +" left join tblscreturnhd e on d.strDNCode=e.strSCDNCode "
		// +" left join tblproductmaster f on b.strProdCode=f.strProdCode left join tblpartymaster g on c.strSCCode=g.strPCode "
		// +" left join tblscreturndtl h on e.strSRCode=h.strSRCode "
