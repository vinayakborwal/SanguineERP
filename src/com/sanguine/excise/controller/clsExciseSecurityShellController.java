...
@Controller
public class clsExciseSecurityShellController {

	@Autowired
	private clsSecurityShellService objSecurityShellService;

	@RequestMapping(value = "/frmExciseSecurityShell", method = RequestMethod.GET)
	public ModelAndView funOpenSecurityShellForm(@ModelAttribute("command") @Valid clsSecurityShellBean objBean, BindingResult result, Map<String, Object> model, HttpServletRequest request) {
		...
		if ("2".equalsIgnoreCase(urlHits)) {
			return new ModelAndView("frmExciseSecurityShell_1");
		} else if ("1".equalsIgnoreCase(urlHits)) {
			return new ModelAndView("frmExciseSecurityShell");
		} else {
			return new ModelAndView("frmExciseSecurityShell");
		}
